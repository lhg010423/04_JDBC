...
public class CommentView {
	 ...
	 * @param boardNo
	 */
	public void commentMenu(int boardNo) throws Exception{
		int input = -1;
		try {
			System.out.println("\n=== 댓글 기능 ===\n");
	 ...
	 * @param boardNo
	 */
	private void insertComment(int boardNo) throws Exception{
		...
		try {
			int result = service.insertComment(boardNo, sb.toString(), Session.loginMember.getMemberNo());
											// 현재 게시글번호, 등록할 댓글내용, 로그인한 사람의 회원번호
	...
	private void updateComment(int boardNo) {
		...
		try {
			...
			int check = service.checkCommenttNo(commentNo, boardNo, Session.loginMember.getMemberNo());
