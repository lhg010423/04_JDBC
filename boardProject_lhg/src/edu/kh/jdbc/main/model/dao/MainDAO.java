...
public class MainDAO {
	...
	private ResultSet rs; // SELECT 수행 결과 저장, 다른거 저장 안함
	
	private Properties prop;
	
	
	
	// 기본생성자 DAO 객체가 생성될 떄 xml 파일 읽어와 Properties 객체 저장
	public MainDAO() {
	...
	public Member login(Connection conn, String memberId, String memberPw) throws Exception{
		...
		try {
			...
			rs = pstmt.executeQuery(); // SELECT 수행 후 결과 반환
			
			// 3. 조회 결과를 1행씩 접근해서 얻어오기
			// 로그인 결과는 무조건 1행만 나오니 while문 말고 if문을 쓴다
			if(rs.next()) {
	...
	public int idDuplicationCheck(Connection conn, String memberId) throws Exception{
		...
		try {
			...
			// sql 결과가 행이 1개이고 값도 1 or 0 이다, 행이 1개라 while쓸필요 없음 if문씀
